# 富岳 環境構築
ここでは、`vascular-cfd-pipeline`をスーパーコンピュータ「富岳」上で実行できるようにするためのセットアップ手順を説明します。<br>
※ 富岳アカウントの発行は済んでいるものとします。

## ログインノードへのアクセス
まずログインノード(`cascadelake`)へアクセスする。方法は2通り
+ [OpenOnDemmend](https://www.fugaku.r-ccs.riken.jp/)でのウェブアクセス
+ CLIからSSH接続

どちらもアクセス結果は同じ。<br>
上のリンクは富岳アカウントを発行したうえでクライアント証明書をブラウザに登録していないと開けない。手順は[スーパーコンピュータ「富岳」スタートアップガイド](https://www.hpci-office.jp/fugaku/user-info/user-guide.pdf)を参照してください。

## 自分の作業環境へ移動する
ログインノードに入れたら、所属するグループ(`hp000000`)のデータ領域(/data)へ移動する。<br>
※ これ以降、GroupID(`hp000000`)やUserID(`u00000`)は各自のものに読み替えてください。
``` bash
pwd     # 出力は /home/u00000
cd /vol0201/data/hp000000
```
あるいは、
``` bash
cd /vol0002/mdt1/data/hp000000
```
どちらも同じ場所を指す。<br>
``` bash
ls
# u00001  u00002  u00003 u00004  u00005  u00006  u00007  u00008
```
ここに課題参加者の作業ディレクトリがある。自分専用のディレクトリを作り、その中で作業をする。<br>
以下のコマンドで権限の確認ができる
``` bash
ls -ld .
# 出力は drwxrws--T 11 root hp000000 4096 Dec 22 19:24 .
```
...
## 計算ノードへ移動する
ここでは、ログインノード上の自身のデータ領域から、計算ノード(`a64fx`)へ入る方法を説明する。<br>
以下のインタラクティブコマンドを実行する。
``` bash
pwd    # /vol0002/mdt1/data/hp000000
pjsub --interact -g hp000000 -x PJM_LLIO_GFSCACHE=/vol0002:/vol0004 -L "node=1,elapse=00:30:00" --sparam wait-time=300
```
各オプションコマンドの意味
| command | 意味 | 備考 |
|---------|------|------|
...
| `--sparam wait-time=300` | 待ち時間の設定 | ノードが空くまで300秒待つ |
...
3. 使いたいOSSをロードする
   ...
   1. **OpenFOAM-v2412**
      ...
      ```
      **※並列でソルバを実行するサンプルケースを`docs/fugaku/OpenFOAM/simpleFoam_fixedFlowRate_gmshParallel`に置きました**

      **※ 注意 !!**. **`Spack load`で使えるようになるOpenFOAMは、dynamicCodeおよびカスタムソルバが使えません。**

      **これらの機能を使いたい場合、OpenFOAMをソースからビルドする必要があります。その手順については[こちら](Build_OpenFOAM_on_Fugaku.md)を参考にしてください**
   ...
   3. **Python3.13.5** <br>
      ...
      **Python3.13**を使いたい場合は、以下のコマンドを実行する。
      ...
      で以下のように返ってくる
      ``` bash
      [u00000@a25-2008c u00000]$ spack load python@3.13.5 ==> Error: python@3.13.5 matches multiple packages.
      Matching packages:
...
4. この設定を行って以降は、bashの接続を切って、再度計算ノードに入ったときでも、
   ``` bash
   . /vol0004/apps/oss/spack/share/spack/setup-env.sh
...
1. `vascular-cfd-pipeline/docs/fugaku/`内の`build_gmsh.sh`および`run_python_gmsh.sh`を、自分の作業ディレクトリ`/vol0002/mdt1/data/hp000000/u00000/`にコピーする
2. gmshをソースビルドするジョブスクリプトを実行
   ...
   **補足** : もし`Permission denied`されるなどErrorが出たら、
   ...
   # 修正前
   config:
     install_tree: /vol0003/hp000000/data/u00000/spack/opt/spack
   # 修正後
   config:
     install_tree: /volmd0002/mdt1/data/hp000000/u00000/spack/opt/spack
   ```
...
4. **計算ノードに入って**、`venv`仮想環境を作り、必要なライブラリをインストールする
   ``` bash
   cd /vol0002/mdt1/data/hp000000/u00000/vascular-cfd-pipeline
   pjsub --interact -g hp000000 -x PJM_LLIO_GFSCACHE=/vol0002:/vol0004 -L "node=1,elapse=00:30:00" --sparam wait-time=300
   . /vol0004/apps/oss/spack/share/spack/setup-env.sh
...
4. gmshをspackにインストールする
   ...
   . /home/u00000/spack/share/spack/setup-env.sh       #u00000の部分は自分のuidに書き換える
